...
(function timelineSystem() {
  ...
  // ── Eye click also collapses the dock to just the eye ─────
  // The eye (panel-tog, first dock child) already toggles panel
  // visibility; we mirror that into a dock-level collapse so all four
  // circles disappear into the lone eye when chrome is hidden.
  const panelTogBtn = document.getElementById('panelTog');
  panelTogBtn?.addEventListener('click', () => {
    if (!dock) return;
    // panel-tog--panels-hidden is set by the panelSystem after toggling.
    // Read AFTER the panel-tog handler runs (microtask) so we see the
    // post-flip state.
    queueMicrotask(() => {
      const hidden = panelTogBtn.classList.contains('panel-tog--panels-hidden');
      dock.classList.toggle('app-dock--collapsed', hidden);
    });
  });
